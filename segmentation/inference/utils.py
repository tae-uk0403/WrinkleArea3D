...
def preprocess_image_for_test(image_path, target_size=(512, 512)):
    ...
    image_tensor = (
        torch.from_numpy(clahe_image).float().unsqueeze(0) / 255.0
    )  # 0-1 범위로 정규화
...
def segmented_area(image_path):
    ...
    visualize_and_save_results(image_path, predicted_masks, output_dir)

    print("predicted_masks is : ", predicted_masks.shape)
    depth_coordinates = convert_to_depth_coordinates(
        predicted_masks, 1, seg_image_size=(512, 512), depth_map_size=(256, 192)
    )
...
def visualize_and_save_results(image_path, predicted_masks, output_dir):
    # 이미지 로드
    ...
    for mask in predicted_masks:
        resized_mask = cv2.resize(mask, (1440, 1920), interpolation=cv2.INTER_NEAREST)
    ...
    for resized_mask in resized_masks:
        ...
        colored_mask[resized_mask > 0] = [0, 0, 255]  # 빨간색으로 설정
        ...
        contours, _ = cv2.findContours(
            resized_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        cv2.drawContours(image, contours, -1, (0, 0, 255), 2)  # 흰색 경계선
...
def convert_to_depth_coordinates(
    predicted_masks,
    segmentation_class,
    seg_image_size=(512, 512),
    depth_map_size=(256, 192),
):
    # predicted_masks에서 특정 클래스의 픽셀 좌표 찾기
    seg_coords = np.argwhere(
        predicted_masks[0] == segmentation_class
    )  # 클래스의 픽셀 좌표
...
def visualize_depth_coordinates(depth_coordinates, seg_image_size=(512, 512)):
    ...
    plt.scatter(
        depth_x, depth_y, c="red", marker="o", s=10
    )  # 깊이 좌표를 빨간 점으로 표시
